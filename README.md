...
- 支持从 `磁力1链接` 的 `dn=` 提取本地别名，用视频文件名反查 CSV 番号
...
> **磁力别名索引**：服务启动时如果检测到数据库里已有影片、但磁力别名索引为空，会自动重建一次别名映射，无需手动删库。
...
| 磁力1链接 | 可选；若存在会从 `dn=` 提取别名，用于按视频文件名反查番号 | `magnet:?xt=...&dn=NBES-059-C` |
...
- `磁力1链接` 仅用于建立本地别名映射，不会直接返回给 Jellyfin
- `dn` 中会自动清洗常见噪声，例如：
  `MOGI-154.torrent -> MOGI-154`
  `第一會所新片@SIS001@NAMH-056-4K.TORRENT.无码破解 -> NAMH-056-4K`
  `START-498-UC.无码破解.torrent -> START-498-UC`
- `FC2PPV-4861886-1` 这类 Jellyfin 传入值会额外尝试候选：
  `FC2PPV-4861886` 和 `FC2-PPV-4861886`
  仅用于查询兜底，不会改写 CSV 原始番号
...
1. 先查 `磁力1链接` 的 `dn` 别名映射
2. 再按 CSV 里的原始番号走现有本地匹配逻辑
3. 本地仍查不到时，再回退到真实 MetaTube Server
